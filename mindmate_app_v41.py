# mindmate_app_v41.py — PREMIUM landing (VSL preko “glow” kruga) + global latice + flower cursor
# Mikro-interakcije (hover scale, ripple), parallax fog, 3D mozak (Three.js) + fallback,
# nove sekcije: Timeline (Kako radi), Naučna osnova (CBT/ACT/Mindfulness), Rezultati (brojači),
# Mini demo graf (SVG), Privatnost, Testimonials, FAQ, Pricing, Sticky CTA,
# + Početna, Chat (Ollama/OpenAI), Check-in, Analitika (Plotly)
...
# ---------- LANDING (VSL na “glow” + global latice, hover scale, nove sekcije) ----------
LANDING_TEMPLATE = """
...
.h-eyebrow{display:inline-block;padding:6px 10px;border-radius:999px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.08);font-size:12px;margin-bottom:10px}
.h-title{font-size:3.2rem;line-height:1.1;margin:0 0 8px}
.h-sub{color:#9AA3B2;font-size:1.08rem;margin:0}

/* --- Glow orb ispod VSL --- */
.orb-wrap{position:relative; height:120px}
.orb{position:absolute; inset: -160px 0 0 0; margin:auto; width:min(980px,86vw); height:min(980px,86vw);
...
.vsl-shell{max-width:1100px;margin:0 auto 16px;padding:12px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.06);border-radius:18px;
...
.btn{position:relative;overflow:hidden;display:inline-block;padding:14px 18px;border-radius:14px;font-weight:800;text-decoration:none;border:1px solid rgba(255,255,255,.08);cursor:pointer; transition:transform .15s ease}
...
/* --- Testimonials, FAQ, Pricing (isti stil kao ranije) --- */
...
// ===== Mini demo graf (SVG path + labels) =====
...
def render_landing():
    ...
    st_html(html, height=3900, width=1280, scrolling=True)
